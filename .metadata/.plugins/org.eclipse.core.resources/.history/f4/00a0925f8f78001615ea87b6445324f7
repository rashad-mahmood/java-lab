...
public class Logic {
	...
	private SortedMap<String, ArrayList<Transaction>> credits = new TreeMap<String, ArrayList<Transaction>>();
...
/*		int payCount = 0;
	...
	public void quickStatsPrintTotal() {
		...
		for (String key : keySet) {
			...
			double monthEarningTotal = 0.0;
			
			System.out.println(key);
			ArrayList<Transaction> transactions = debits.get(key);
			ArrayList<Transaction> funTransactions = funDebits.get(key);
			ArrayList<Transaction> earnings = credits.get(key);
			
			for (Transaction t : earnings) {
				System.out.println("\t" + t.getName() + " " + t.getAmount());
				monthEarningTotal += t.getAmount();
				earningTotal += t.getAmount();
			}
			System.out.println("\t-------------");
			System.out.println("\tTotal Earnings   => $" + monthEarningTotal);
			System.out.println("\t-------------\n");
		...
		for (CSVRecord csvRecord : parser) {
			...
			if (credit.length() > 0) {
				if (name.contains("FDM GROUP LIMIT  PAY")) {
					...
					putInMap(credits, dateKey, Double.parseDouble(credit), name);
				} else {
